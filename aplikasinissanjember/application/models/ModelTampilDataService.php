...
class ModelTampilDataService extends CI_Model{
	...
	public function get_tampil_dataservice(){
	$this->db->select('*'); 
	...
	public function get_tampil_dataservice_byid($id){
        $this->db->select('*'); 
	    $this->db->from('tb_daftar_service'); 
    	$this->db->join('tb_user', 'tb_daftar_service.id_user = tb_user.id_user', 'inner');
	...
	public function update_tampil_dataservice($id,$data){
            $this->db->where('id_daftar_service', $id);
